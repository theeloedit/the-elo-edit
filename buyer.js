(async function () {
  ...
  function render() {
    ...
    // let people tap a specific dot to jump straight to that photo
  ...
  // Attached once, outside render() — app.innerHTML gets replaced on every
  // render but #app itself never does, so this must not be re-attached each
  // time or listeners stack up and one swipe ends up firing several times.
